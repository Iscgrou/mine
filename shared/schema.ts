 ...
 * 1) Using integer primary keys with autoIncrement for the ID columns
 * 2) Using real type instead of decimal
 * 3) Using integer with mode: "timestamp" for timestamps
 * 4) Using integer for booleans (0/1)
 * 5) For arrays in PostgreSQL, storing as text (parse in code)
 * 6) For jsonb, storing as text (parse in code)
 */
...
export const invoices = sqliteTable("invoices", {
  ...
  invoiceData: text("invoice_data"), // JSON as text
  autoCalculated: integer("auto_calculated").default(1), // boolean as integer
  priceSource: text("price_source").default("representative_rate"),
  telegramSent: integer("telegram_sent").default(0), // boolean as integer
  sentToRepresentative: integer("sent_to_representative").default(0), // boolean as integer
  createdAt: integer("created_at", { mode: "timestamp" })
    .default(sql`CURRENT_TIMESTAMP`),
  updatedAt: integer("updated_at", { mode: "timestamp" })
    .default(sql`CURRENT_TIMESTAMP`),
...
export const crmInteractions = sqliteTable("crm_interactions", {
  ...
  keyTopics: text("key_topics"), // array stored as text
  createdAt: integer("created_at", { mode: "timestamp" })
    .default(sql`CURRENT_TIMESTAMP`)
    .notNull(),
  updatedAt: integer("updated_at", { mode: "timestamp" })
...
export const crmTasks = sqliteTable("crm_tasks", {
  ...
  aiGenerated: integer("ai_generated").default(0), // boolean as integer
  createdAt: integer("created_at", { mode: "timestamp" })
    .default(sql`CURRENT_TIMESTAMP`)
    .notNull(),
  updatedAt: integer("updated_at", { mode: "timestamp" })
...
export const crmCallPreparations = sqliteTable("crm_call_preparations", {
  ...
  aiTalkingPoints: text("ai_talking_points"), // JSON as text
