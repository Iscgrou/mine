...
export class SecureAPIManager {
  ...
  async saveAllAPIKeys(keys: Partial<SecureAPIVault>): Promise<void> {
    await this.acquireLock();
    try {
      const vault = await this.loadVault();
      Object.assign(vault, keys);
      vault.lastAccessed = new Date().toISOString();
      
      await this.saveVault(vault);
      console.log('✅ All API keys secured in vault');
    } finally {
      await this.releaseLock();
    }
  }
  ...
  async initializeVaultSecurity(): Promise<void> {
    console.log('🔐 INITIALIZING SECURE API VAULT');
    console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
    
    // Migrate existing keys from environment
    const vault = await this.loadVault();
    vault.grokApiKey = vault.grokApiKey || process.env.GROK_API_KEY || '';
    vault.sentimentApiKey = vault.sentimentApiKey || process.env.SENTIMENT_API_KEY || '';
    vault.sttApiKey = vault.sttApiKey || process.env.STT_API_KEY || '';
    vault.vertexAIApiKey = vault.vertexAIApiKey || process.env.GOOGLE_AI_STUDIO_API_KEY || '';
    
    await this.saveVault(vault);
    
    console.log('✅ Secure vault initialized');
    console.log('✅ Existing API keys migrated');
    console.log('✅ Ready for Vertex AI key upload');
  }
}
